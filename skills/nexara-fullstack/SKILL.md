...
The UI layer requires the `ui-ux-pro-max` skill (MIT-licensed, by nextlevelbuilder). Install it as-is — do not vendor its contents into Nexara skills:
...
1. **Pick the mode** from `references/backend-modes.md` and build to its patterns. Do not switch modes, ORMs, or auth providers mid-build — if the approval is inconsistent, stop and raise it.
